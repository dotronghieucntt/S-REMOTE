    ...
    try {
        ...
        if ($radioTailscale.Checked) {
            ...
            try {
                ...
                # Verify port 3389
                Start-Sleep -Seconds 5
                $tsPort = Get-NetTCPConnection -LocalPort $RdpPort -State Listen -ErrorAction SilentlyContinue
                if ($tsPort) {
                    Write-Output-Box "[OK] Port $RdpPort is LISTENING"
                    $tsRdpWorking = $true
                } else {
                    ...
                    $tsPort2 = Get-NetTCPConnection -LocalPort $RdpPort -State Listen -ErrorAction SilentlyContinue
                    if ($tsPort2) {
                        Write-Output-Box "[OK] Port $RdpPort is now LISTENING (after deep reset)"
                        $tsRdpWorking = $true
                    } else {
                        ...
                        $tsPort3 = Get-NetTCPConnection -LocalPort $RdpPort -State Listen -ErrorAction SilentlyContinue
                        if ($tsPort3) {
                            Write-Output-Box "[OK] Port $RdpPort LISTENING after listener rebind"
                            $tsRdpWorking = $true
                        } else {
                            Write-Output-Box "[WARNING] Port $RdpPort still not listening"
...
"@ | Set-Content -Path $tsScriptPath -Encoding UTF8 -Force
            ...
            if (-not $tsRdpWorking) {
                ...
                Write-Output-Box "[INFO] A machine restart is required for RDP to work"
                Write-Output-Box "[INFO] Run: shutdown /r /t 0"
            }
